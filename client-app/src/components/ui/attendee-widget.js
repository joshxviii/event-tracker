        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12}}>
            ...
            {attendees.slice(0, maxAvatars).map((a, idx) => {
                ...
                <div key={key} title={name} onClick={() => navigate(`/user/${a._id}`)} style={{ width: 32, height: 32, borderRadius: 6, overflow: 'hidden', flex: '0 0 auto' }}>
                    {pic ? (
                        <img src={pic} alt={name} className="profilePicture" style={{ borderRadius: 6 }} />
                    ) : (
                        <div className="nullPicture" style={{ borderRadius: 6, fontSize: 12 }}>{(name || '').charAt(0).toUpperCase()}</div>
                    )}
                </div>
                );
            })}
